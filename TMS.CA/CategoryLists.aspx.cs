    ...
    public partial class CategoryLists : System.Web.UI.Page
    {
        ...
        private void DeleteRecord(string Id)
        {
            try
            {
                ...
                {
                    using (MySqlCommand cmd = new MySqlCommand("DELETE FROM Category WHERE CategoryId=@CategoryId"))
                    {
        ...
        private void BindDataById(string Id)
        {
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(databaseConnection))
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM Category where CategoryId=@CategoryId"))
                    {
                        using (MySqlDataAdapter sda = new MySqlDataAdapter())
                        {
                            cmd.Parameters.AddWithValue("@CategoryId", Id);
        ...
        private void BindData()
        {
            try
            {
                ...
                htmldata += "<table class='table table-bordered table-striped mt-3' id='CategoryList'>" +
                    "<thead>" +
                        "<tr>" +
                            "<th>No</th>" +
                            "<th>Category Name</th>" +
                        ...
                        using (MySqlDataAdapter sda = new MySqlDataAdapter())
                        {
                            ...
                            using (DataTable dt = new DataTable())
                            {
                                ...
                                for (int i = 0; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    "<a href=CategoryLists.aspx?Id=" + dt.Rows[i]["CategoryId"] + "&Action=Edit class='btn btn-link text-theme p-1'><i class='fa fa-pencil'></i></button>" +
                                    "<a href=CategoryLists.aspx?Id=" + dt.Rows[i]["CategoryId"] + "&Action=Delete class='btn btn-link text-danger p-1'><i class='fas fa-trash'></i></button>" +
                                "</td></tr>";
                                }
                            }
                        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(databaseConnection))
                {
                    using (MySqlCommand cmd = new MySqlCommand("INSERT INTO Category (Name,Status) VALUES (@Name, @Status)"))
                    {
                        using (MySqlDataAdapter sda = new MySqlDataAdapter())
                        {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(databaseConnection))
                {
                    using (MySqlCommand cmd = new MySqlCommand("UPDATE Category SET Name = @Name, Status = @Status  WHERE CategoryId = @CategoryId"))
                    {
                        using (MySqlDataAdapter sda = new MySqlDataAdapter())
                        {
